...
import { persistFileToWalrus, persistMessageToWalrus, type WalrusStorageStatus } from "./walrusMemory.js";
...
export async function backfillWalrusFiles(params: {
  ...
  for (const { file } of rows) {
    try {
      ...
      if (!response.ok) throw new Error(`bucket read failed with ${response.status} ${response.statusText}`);
