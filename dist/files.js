...
export async function downloadSlackFile(url, botToken) {
...
export async function captureSlackFile(params) {
    ...
    if (file.size > MAX_FILE_SIZE_BYTES) {
        console.error(`captureSlackFile: skipping oversize Slack file ${file.id} (${file.name}) for message ${messageId}: ` +
    ...
    try {
        ...
        console.error(`captureSlackFile: failed to capture Slack file ${file.id} (${file.name}) for message ${messageId}:`, error);
