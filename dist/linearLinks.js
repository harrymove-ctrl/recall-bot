...
const LINEAR_ISSUE_URL_RE = /(?<![a-zA-Z0-9-])linear\.app\/([a-zA-Z0-9](?:[a-zA-Z0-9-]{0,62}[a-zA-Z0-9])?)\/issue\/([a-zA-Z][a-zA-Z0-9]{1,9}-\d+)/g;
/**
 ...
 */
export function extractLinearIssueRefs(text) {
    const refs = new Map();
...
export function linearIssueUrl(ref) {
...
export async function recordLinearIssueLinks(params) {
    ...
    for (const ref of refs) {
        try {
            ...
            console.error(`recordLinearIssueLinks: failed to record ${ref.issueIdentifier} for namespace ${namespaceId}:`, error);
        }
    }
}
//# sourceMappingURL=linearLinks.js.map