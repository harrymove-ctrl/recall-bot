...
import { DASHBOARD_COOKIE_NAME, requireDashboardSession } from "./auth.js";
...
export function createDashboardApiRouter(db, sessionSecret) {
    const router = Router();
    ...
    router.get("/namespaces", auth, async (req, res) => {
        const rows = await db
            .select()
            .from(namespaces)
    ...
    router.get("/namespaces/:id/messages", auth, async (req, res) => {
    ...
    router.patch("/namespaces/:id", auth, async (req, res) => {
        ...
        if (label !== undefined)
            update.label = typeof label === "string" && label.length > 0 ? label : null;
        if (status === "archived")
    ...
    router.post("/users/:id/revoke-key", auth, async (req, res) => {
