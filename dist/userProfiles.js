...
async function upsertProfile(db, workspaceId, slackUserId, displayName, avatarUrl) {
    const now = new Date();
    await db
 ...
 */
export async function resolveDisplayNames(db, workspaceId, slackUserIds) {
    ...
    try {
        ...
        console.error(`resolveDisplayNames: unexpected failure resolving profiles for workspace ${workspaceId}, falling back to raw ids:`, error);
        const fallback = new Map();
        for (const id of uniqueIds)
            fallback.set(id, { displayName: null, avatarUrl: null });
        return fallback;
    }
}
async function resolveDisplayNamesUnguarded(db, workspaceId, uniqueIds) {
    ...
    for (const slackUserId of uniqueIds) {
        ...
        try {
            ...
            if (code && AUTH_CLASS_ERROR_CODES.has(code)) {
                ...
                console.warn(`resolveDisplayNames: auth-class Slack error "${code}" for workspace ${workspaceId} — ` +
