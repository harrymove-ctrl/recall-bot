...
export async function handleAppMention(params) {
    ...
    try {
        await client.chat.postMessage({
            ...
            thread_ts: threadTs,
            text: `Got it — this thread is now saved. Run \`/recall-key\` in a DM with me to get your delegate key, ` +
...
export async function handleMessage(params) {
    ...
    if (!message.thread_ts || !message.user)
        return; // not a thread reply, or a subtype we don't track
    const [namespace] = await db
        .select()
        .from(namespaces)
        .where(and(eq(namespaces.workspaceId, workspaceId), eq(namespaces.channelId, message.channel), eq(namespaces.threadTs, message.thread_ts), eq(namespaces.status, "active")));
    if (!namespace)
        return; // not a thread we're watching
...
export function isCapturableMessageSubtype(subtype) {
    if (subtype === undefined)
        return true;
    return CAPTURABLE_MESSAGE_SUBTYPES.has(subtype);
}
export function registerEventHandlers(app, db) {
    ...
    app.message(async ({ message, context }) => {
        const teamId = context.teamId;
        if (!teamId || !isCapturableMessageSubtype(message.subtype))
            return;
        const workspace = await resolveWorkspaceByTeamId(db, teamId);
        if (!workspace)
            return;
