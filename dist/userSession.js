...
export function createUserSessionCookie(workspaceId, slackUserId, secret, maxAgeMs = DEFAULT_MAX_AGE_MS) {
    const payload = { workspaceId, slackUserId, exp: Date.now() + maxAgeMs };
    const payloadB64 = Buffer.from(JSON.stringify(payload), "utf8").toString("base64url");
    const sig = sign(payloadB64, secret);
    return `${payloadB64}.${sig}`;
}
export function verifyUserSessionCookie(cookieValue, secret) {
    if (!cookieValue)
        return null;
    ...
    if (typeof payload !== "object" || payload === null)
        return null;
    const { workspaceId, slackUserId, exp } = payload;
