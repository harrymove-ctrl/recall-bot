 ...
 */
const REQUIRED_BUCKET_ENV_VARS = ["BUCKET", "ACCESS_KEY_ID", "SECRET_ACCESS_KEY", "ENDPOINT"];
const SLACK_AUTH_STATE_MAX_AGE_MS = 1000 * 60 * 10;
function signSlackAuthState(secret, now = Date.now()) {
    const payload = Buffer.from(JSON.stringify({ ts: now }), "utf8").toString("base64url");
    const signature = crypto.createHmac("sha256", secret).update(payload).digest("base64url");
    return `${payload}.${signature}`;
}
function verifySlackAuthState(state, secret, now = Date.now()) {
    if (!state)
        return false;
...
export function buildApp(database) {
    for (const name of REQUIRED_BUCKET_ENV_VARS)
        requireEnv(name);
    const app = express();
    ...
    app.get("/auth/slack/callback", async (req, res) => {
        ...
        try {
            ...
            if (!workspace) {
                // User authenticated with Slack but workspace isn't set up yet.
                // Redirect back to dashboard with an error — not /slack/install (only admins need that).
            ...
            const cookie = createUserSessionCookie(workspace.id, userId, userSessionSecret, 1000 * 60 * 60 * 24 * 7);
    ...
    if (userSessionSecret === dashboardSessionSecret) {
        throw new Error("USER_SESSION_SECRET must not equal DASHBOARD_SESSION_SECRET — generate a second, independent secret (openssl rand -hex 32).");
    }
